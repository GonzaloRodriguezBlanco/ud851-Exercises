...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        Toast error = Toast.makeText(getContext(), R.string.error_msg_invalid_size_preference, Toast.LENGTH_SHORT);

        String prefSizeKey = getString(R.string.pref_size_key);

        if (prefSizeKey.equals(preference.getKey())) {
            String stringSizeMultiplier = ((String) newValue).trim();

            try {
                float sizeMultiplier = Float.parseFloat(stringSizeMultiplier);

                if (sizeMultiplier <= 0 || sizeMultiplier > 3) {
                    error.show();
                    return false;
                }
